# OpenMehdi ACP Bridge

This document describes how the OpenMehdi ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`openmehdi acp` exposes an ACP agent over stdio and forwards prompts to a running
OpenMehdi Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a OpenMehdi Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `openmehdi acp` over stdio.

Example config:

```bash
openmehdi config set gateway.remote.url wss://gateway-host:18789
openmehdi config set gateway.remote.token <token>
```

Example run:

```bash
openmehdi acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
openmehdi acp --session agent:main:main
openmehdi acp --session agent:design:main
openmehdi acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “OpenMehdi ACP” to start a thread.

## Execution Model

- ACP client spawns `openmehdi acp` and speaks ACP messages over stdio.
...
```bash
openmehdi acp --session agent:main:main
openmehdi acp --session-label "support inbox"
openmehdi acp --reset-session
```
...
`openmehdi acp` resolves the Gateway URL and auth from CLI flags or config:

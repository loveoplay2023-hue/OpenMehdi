---
summary: "Use Synthetic's Anthropic-compatible API in OpenMehdi"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. OpenMehdi registers it as the
...
Note: OpenMehdi's Anthropic client appends `/v1` to the base URL, so use

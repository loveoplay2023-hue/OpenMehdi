---
summary: "Use Venice AI privacy-focused models in OpenMehdi"
read_when:
  - You want privacy-focused inference in OpenMehdi
...
```bash
openmehdi onboard --auth-choice venice-api-key
```
...
```bash
openmehdi chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, OpenMehdi shows all available Venice models. Pick based on your needs:
...
```bash
openmehdi models set venice/claude-opus-45
openmehdi models set venice/llama-3.3-70b
```

List all available models:

```bash
openmehdi models list | grep venice
```

## Configure via `openmehdi configure`

1. Run `openmehdi configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenMehdi automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openmehdi chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
openmehdi chat --model venice/claude-opus-45

# Use uncensored model
openmehdi chat --model venice/venice-uncensored

# Use vision model with image
openmehdi chat --model venice/qwen3-vl-235b-a22b

# Use coding model
openmehdi chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `openmehdi models list` to see currently available models. Some models may be temporarily offline.

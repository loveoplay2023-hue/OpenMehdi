---
summary: "Use MiniMax M2.1 in OpenMehdi"
read_when:
  - You want MiniMax models in OpenMehdi
...
```bash
openmehdi plugins enable minimax-portal-auth  # skip if already loaded.
openmehdi gateway restart  # restart if gateway is already running
openmehdi onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/openmehdi/openmehdi/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `openmehdi.json`:
...
- Use `openmehdi models list` and `openmehdi models set minimax/MiniMax-M2.1` to switch.
...
- Running `openmehdi configure` and selecting **MiniMax M2.1**, or

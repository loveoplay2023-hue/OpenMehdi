...
OpenMehdi integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenMehdi owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenMehdi spawns `imsg rpc` as a child process (legacy iMessage integration).

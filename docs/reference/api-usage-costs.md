...
OpenMehdi features that can generate provider usage or paid API calls.
...
- `openmehdi status --usage` and `openmehdi channels list` show provider **usage windows**
...
OpenMehdi can pick up credentials from:
...
- `openmehdi status --usage`
- `openmehdi models status --json`
...
`openmehdi models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.

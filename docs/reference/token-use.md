---
summary: "How OpenMehdi builds prompt context and reports token usage + costs"
read_when:
...
OpenMehdi tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OpenMehdi assembles its own system prompt on every run. It includes:
...
For images, OpenMehdi downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from OpenMehdi session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `openmehdi status --usage` and `openmehdi channels list` show
...
`cacheWrite`. If pricing is missing, OpenMehdi shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. OpenMehdi can
...
Anthropic's 1M context window is currently beta-gated. OpenMehdi can inject the

...
description: "Complete field-by-field reference for ~/.openmehdi/openmehdi.json"
---

# Configuration Reference

Every field available in `~/.openmehdi/openmehdi.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OpenMehdi uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `openmehdi doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["openmehdi-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `openmehdi plugins install @openmehdi/mattermost`.
...
OpenMehdi spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@openmehdi"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `openmehdi.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, OpenMehdi auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OpenMehdi assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.openmehdi/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.openmehdi/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@openmehdi"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:openmehdi`   | All built-in tools (excludes provider plugins)                                           |
...
OpenMehdi uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.openmehdi/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `OPENMEHDI_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `openmehdi onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `openmehdi onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `openmehdi onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `openmehdi onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `openmehdi onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `openmehdi onboard --auth-choice minimax-api`.
...
- Loaded from `~/.openmehdi/extensions`, `<workspace>/.openmehdi/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or OPENMEHDI_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `OPENMEHDI_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.openmehdi-dev` + port `19001`), `--profile <name>` (uses `~/.openmehdi-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.openmehdi/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-openmehdi-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `OPENMEHDI_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or OPENMEHDI_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__openmehdi__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openmehdi__/a2ui/`
...
- Also serves A2UI at `/__openmehdi__/a2ui/`.
...
- Hostname defaults to `openmehdi`. Override with `OPENMEHDI_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.openmehdi/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `openmehdi dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.openmehdi/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${OPENMEHDI_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.openmehdi/credentials/oauth.json`.
...
- Default log file: `/tmp/openmehdi/openmehdi-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `openmehdi doctor --fix` can strip unknown keys).

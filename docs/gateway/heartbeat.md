...
- During heartbeat runs, OpenMehdi treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openmehdi sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OpenMehdi skips the heartbeat run entirely (no model call).
...
```bash
openmehdi system event --text "Check for urgent follow-ups" --mode now
```

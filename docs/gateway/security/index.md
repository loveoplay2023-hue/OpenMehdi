...
## Quick check: `openmehdi security audit`
...
```bash
openmehdi security audit
openmehdi security audit --deep
openmehdi security audit --fix
openmehdi security audit --json
```

It flags common footguns (Gateway auth exposure, browser control exposure, elevated allowlists, filesystem permissions).

OpenMehdi is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, OpenMehdi also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.openmehdi/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.openmehdi/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.openmehdi/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.openmehdi/credentials/oauth.json`
...
| -------------------------------------------- | ------------- | -------------------------------------------------------- | ------------------------------------------------ | -------- |
| `fs.state_dir.perms_world_writable`          | critical      | Other users/processes can modify full OpenMehdi state     | filesystem perms on `~/.openmehdi`                | yes      |
| `fs.config.perms_writable`                   | critical      | Others can change auth/tool policy/config                | filesystem perms on `~/.openmehdi/openmehdi.json`  | yes      |
...
`openmehdi security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${OPENMEHDI_GATEWAY_PASSWORD}
```
...
OpenMehdi stores session transcripts on disk under `~/.openmehdi/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.openmehdi` (see the audit section below). If you need
...
OpenMehdi can refresh the skills list mid-session:
...
- If you install plugins from npm (`openmehdi plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.openmehdi/extensions/<pluginId>/` (or `$OPENMEHDI_STATE_DIR/extensions/<pluginId>/`).
  - OpenMehdi uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
openmehdi pairing list <channel>
openmehdi pairing approve <channel> <code>
```
...
By default, OpenMehdi routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
OpenMehdi has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.openmehdi/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.openmehdi or your logs.”

## Unsafe external content bypass flags

OpenMehdi includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.openmehdi/openmehdi.json`: `600` (user read/write only)
- `~/.openmehdi`: `700` (user only)

`openmehdi doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `OPENMEHDI_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__openmehdi__/canvas/` and `/__openmehdi__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_openmehdi-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `OPENMEHDI_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `openmehdi doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `OPENMEHDI_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `OPENMEHDI_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), OpenMehdi
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. OpenMehdi verifies the identity by resolving the
...
- OpenMehdi will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.openmehdi/` (or `$OPENMEHDI_STATE_DIR/`) may contain secrets or private data:

- `openmehdi.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `openmehdi status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@openmehdi", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.openmehdi/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `openmehdi` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only OpenMehdi clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default OpenMehdi limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `openmehdi gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `OPENMEHDI_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/openmehdi/openmehdi-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.openmehdi/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `openmehdi security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + OpenMehdi version
...
Found a vulnerability in OpenMehdi? Please report responsibly:

1. Email: [security@openmehdi.ai](mailto:security@openmehdi.ai)

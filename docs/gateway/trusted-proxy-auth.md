...
read_when:
  - Running OpenMehdi behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of OpenMehdi
...
- You run OpenMehdi behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. OpenMehdi checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. OpenMehdi extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://openmehdi.example.com
    to: http://openmehdi-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
openmehdi.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy openmehdi:18789 {
...
location / {
    ...
    proxy_pass http://openmehdi:18789;
...
`openmehdi security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update OpenMehdi config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `openmehdi security audit` and review findings

...
- Jobs persist under `~/.openmehdi/cron/` so restarts don’t lose schedules.
...
openmehdi cron list
openmehdi cron run <job-id>
openmehdi cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.openmehdi/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `openmehdi cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, OpenMehdi applies a
...
If `delivery` is omitted for isolated jobs, OpenMehdi defaults to `announce`.
...
- Job store: `~/.openmehdi/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.openmehdi/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.openmehdi/cron/jobs.json",
...
openmehdi cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
openmehdi cron edit <jobId> --agent ops
openmehdi cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
openmehdi cron run <jobId>
openmehdi cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
openmehdi cron edit <jobId> \
...
```bash
openmehdi cron edit <jobId> --exact
```
...
```bash
openmehdi system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`openmehdi system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OPENMEHDI_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OpenMehdi applies exponential retry backoff for recurring jobs after consecutive errors:

...
- `/usage cost` in chats: local cost summary aggregated from OpenMehdi session logs.
- CLI: `openmehdi status --usage` prints a full per-provider breakdown.
- CLI: `openmehdi channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).

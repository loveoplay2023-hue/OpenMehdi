...
```bash
openmehdi plugins install @openmehdi/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
openmehdi plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenMehdi" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openmehdi pairing list nextcloud-talk`
  - `openmehdi pairing approve nextcloud-talk <CODE>`

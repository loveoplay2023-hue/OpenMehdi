---
summary: "Mattermost bot setup and OpenMehdi config"
read_when:
...
```bash
openmehdi plugins install @openmehdi/mattermost
```

Local checkout (when running from a git repo):

```bash
openmehdi plugins install ./extensions/mattermost
```
...
4. Configure OpenMehdi and start the gateway.
...
- Approve via:
  - `openmehdi pairing list mattermost`
  - `openmehdi pairing approve mattermost <CODE>`
...
Use these target formats with `openmehdi message send` or cron/webhooks:

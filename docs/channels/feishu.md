...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OpenMehdi to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
openmehdi plugins install @openmehdi/feishu
```

Local checkout (when running from a git repo):

```bash
openmehdi plugins install ./extensions/feishu
```
...
If you just installed OpenMehdi, run the wizard:
...
2. Configuring app credentials in OpenMehdi
...
1. You already ran `openmehdi channels add` for Feishu
2. The gateway is running (`openmehdi gateway status`)
...
```bash
openmehdi pairing approve feishu <CODE>
```
...
2. Run `openmehdi logs --follow` and look for `chat_id`
...
2. Run `openmehdi logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `openmehdi gateway status`  | Show gateway status           |
| `openmehdi gateway install` | Install/start gateway service |
| `openmehdi gateway stop`    | Stop gateway service          |
| `openmehdi gateway restart` | Restart gateway service       |
| `openmehdi logs --follow`   | Tail gateway logs             |
...
4. Check logs: `openmehdi logs --follow`
...
5. Ensure the gateway is running: `openmehdi gateway status`
6. Check logs: `openmehdi logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.openmehdi/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.openmehdi/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [

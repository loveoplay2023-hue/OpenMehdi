...
read_when:
  - Setting up Twitch chat integration for OpenMehdi
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenMehdi connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openmehdi plugins install @openmehdi/twitch
```

Local checkout (when running from a git repo):

```bash
openmehdi plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENMEHDI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openmehdi", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENMEHDI_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENMEHDI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```

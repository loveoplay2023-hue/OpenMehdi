...
Tlon is a decentralized messenger built on Urbit. OpenMehdi connects to your Urbit ship and can
...
```bash
openmehdi plugins install @openmehdi/tlon
```

Local checkout (when running from a git repo):

```bash
openmehdi plugins install ./extensions/tlon
```
...
By default, OpenMehdi blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `openmehdi message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenMehdi replies in-thread.

...
Matrix is an open, decentralized messaging protocol. OpenMehdi connects as a Matrix **user**
...
```bash
openmehdi plugins install @openmehdi/matrix
```

Local checkout (when running from a git repo):

```bash
openmehdi plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `openmehdi plugins install @openmehdi/matrix`
   - From a local checkout: `openmehdi plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenMehdi calls the same
     login endpoint, stores the access token in `~/.openmehdi/credentials/matrix/credentials.json`,
...
- On first connection, OpenMehdi requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenMehdi logs a warning.
...
`~/.openmehdi/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `openmehdi pairing list matrix`
  - `openmehdi pairing approve matrix <CODE>`
...
- On startup, OpenMehdi resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.

...
service via the external `openmehdi` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`OPENMEHDI_PROFILE`; legacy `com.openmehdi.*` is supported).

---
summary: "OpenMehdi on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenMehdi on a Raspberry Pi
  - Running OpenMehdi on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenMehdi on Raspberry Pi

## Goal

Run a persistent, always-on OpenMehdi Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OpenMehdi features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openmehdi

# Enable if not
sudo systemctl enable openmehdi

# Start on boot
sudo systemctl start openmehdi
```
...
```bash
# Check logs
journalctl -u openmehdi --no-pager -n 100

# Common fix: rebuild
cd ~/openmehdi  # if using hackable install
npm run build
sudo systemctl restart openmehdi
```

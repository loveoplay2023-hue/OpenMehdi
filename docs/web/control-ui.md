...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/openmehdi`)
...
If the page fails to load, start the Gateway first: `openmehdi gateway`.
...
you revoke it with `openmehdi devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.openmehdi/openmehdi.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenMehdi
...
OpenMehdi **blocks** Control UI connections without device identity.

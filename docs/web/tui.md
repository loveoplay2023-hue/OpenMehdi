...
```bash
openmehdi tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `openmehdi tui --deliver`
...
- Check the Gateway logs: `openmehdi logs --follow`.
- Confirm the agent can run: `openmehdi status` and `openmehdi models status`.
...
- No agents in picker: check `openmehdi agents list` and your routing config.

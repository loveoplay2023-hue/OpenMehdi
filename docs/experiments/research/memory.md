...
read_when:
  - Designing workspace memory (~/.openmehdi/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenMehdi integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.openmehdi/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.openmehdi/workspace` as canonical human-readable memory.
...
```
~/.openmehdi/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OpenMehdi**, but keep a separable core library.

### Why integrate into OpenMehdi?

- OpenMehdi already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `openmehdi memory recall "…" --k 25 --since 30d`
  - `openmehdi memory reflect --since 7d`
...
Pragmatic take for `~/.openmehdi/workspace`:

...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenMehdi
...
Running OpenMehdi via Node with `tsx` fails at startup with:

```
[openmehdi] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openmehdi status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch openmehdi.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node openmehdi.mjs status` works on Node 25.

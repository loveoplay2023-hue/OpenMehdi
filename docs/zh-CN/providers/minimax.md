---
read_when:
  - 你想在 OpenMehdi 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 OpenMehdi 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
openmehdi plugins enable minimax-portal-auth  # 如果已加载则跳过
openmehdi gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/openmehdi/openmehdi/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `openmehdi models list` 和 `openmehdi models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `openmehdi configure` 并选择 **MiniMax M2.1**，或

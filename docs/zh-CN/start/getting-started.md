...
最快聊天：打开 Control UI（无需渠道设置）。运行 `openmehdi dashboard` 并在浏览器中聊天，或在 Gateway 网关主机上打开 `http://127.0.0.1:18789/`。文档：[Dashboard](/web/dashboard) 和 [Control UI](/web/control-ui)。

推荐路径：使用 **CLI 新手引导向导**（`openmehdi onboard`）。它设置：
...
- Node `>=22`
- `pnpm`（可选；如果从源代码构建则推荐）
- **推荐：**Brave Search API 密钥用于网页搜索。最简单的方式：`openmehdi configure --section web`（存储 `tools.web.search.apiKey`）。参见 [Web 工具](/tools/web)。
...
- OAuth 凭证（旧版导入）：`~/.openmehdi/credentials/oauth.json`
...
如果你还没有全局安装，从仓库通过 `pnpm openmehdi ...` 运行新手引导步骤。`pnpm build` 也会打包 A2UI 资源；如果你只需要运行那个步骤，使用 `pnpm canvas:a2ui:bundle`。

Gateway 网关（从此仓库）：

```bash
node openmehdi.mjs gateway --port 18789 --verbose
```
...
```bash
openmehdi message send --target +15555550123 --message "Hello from OpenMehdi"
```

如果 `openmehdi health` 显示"未配置认证"，回到向导设置 OAuth/密钥认证——没有它智能体将无法响应。

提示：`openmehdi status --all` 是最佳的可粘贴、只读调试报告。
健康探测：`openmehdi health`（或 `openmehdi status --deep`）向运行中的 Gateway 网关请求健康快照。

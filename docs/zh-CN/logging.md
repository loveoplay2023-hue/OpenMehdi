...
你可以在 `~/.openmehdi/openmehdi.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/openmehdi.log"
  }
}
```
...
```bash
openmehdi channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.openmehdi/openmehdi.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/openmehdi/openmehdi-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- OpenMehdi 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
OPENMEHDI_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `openmehdi plugins enable diagnostics-otel` 启用插件。
...
- `openmehdi.tokens`（计数器，属性：`openmehdi.token`、`openmehdi.channel`、`openmehdi.provider`、`openmehdi.model`）
- `openmehdi.cost.usd`（计数器，属性：`openmehdi.channel`、`openmehdi.provider`、`openmehdi.model`）
- `openmehdi.run.duration_ms`（直方图，属性：`openmehdi.channel`、`openmehdi.provider`、`openmehdi.model`）
- `openmehdi.context.tokens`（直方图，属性：`openmehdi.context`、`openmehdi.channel`、`openmehdi.provider`、`openmehdi.model`）

消息流：

- `openmehdi.webhook.received`（计数器，属性：`openmehdi.channel`、`openmehdi.webhook`）
- `openmehdi.webhook.error`（计数器，属性：`openmehdi.channel`、`openmehdi.webhook`）
- `openmehdi.webhook.duration_ms`（直方图，属性：`openmehdi.channel`、`openmehdi.webhook`）
- `openmehdi.message.queued`（计数器，属性：`openmehdi.channel`、`openmehdi.source`）
- `openmehdi.message.processed`（计数器，属性：`openmehdi.channel`、`openmehdi.outcome`）
- `openmehdi.message.duration_ms`（直方图，属性：`openmehdi.channel`、`openmehdi.outcome`）

队列 + 会话：

- `openmehdi.queue.lane.enqueue`（计数器，属性：`openmehdi.lane`）
- `openmehdi.queue.lane.dequeue`（计数器，属性：`openmehdi.lane`）
- `openmehdi.queue.depth`（直方图，属性：`openmehdi.lane` 或 `openmehdi.channel=heartbeat`）
- `openmehdi.queue.wait_ms`（直方图，属性：`openmehdi.lane`）
- `openmehdi.session.state`（计数器，属性：`openmehdi.state`、`openmehdi.reason`）
- `openmehdi.session.stuck`（计数器，属性：`openmehdi.state`）
- `openmehdi.session.stuck_age_ms`（直方图，属性：`openmehdi.state`）
- `openmehdi.run.attempt`（计数器，属性：`openmehdi.attempt`）

### 导出的 span（名称 + 关键属性）

- `openmehdi.model.usage`
  - `openmehdi.channel`、`openmehdi.provider`、`openmehdi.model`
  - `openmehdi.sessionKey`、`openmehdi.sessionId`
  - `openmehdi.tokens.*`（input/output/cache_read/cache_write/total）
- `openmehdi.webhook.processed`
  - `openmehdi.channel`、`openmehdi.webhook`、`openmehdi.chatId`
- `openmehdi.webhook.error`
  - `openmehdi.channel`、`openmehdi.webhook`、`openmehdi.chatId`、`openmehdi.error`
- `openmehdi.message.processed`
  - `openmehdi.channel`、`openmehdi.outcome`、`openmehdi.chatId`、`openmehdi.messageId`、`openmehdi.sessionKey`、`openmehdi.sessionId`、`openmehdi.reason`
- `openmehdi.session.stuck`
  - `openmehdi.state`、`openmehdi.ageMs`、`openmehdi.queueDepth`、`openmehdi.sessionKey`、`openmehdi.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `openmehdi doctor`。

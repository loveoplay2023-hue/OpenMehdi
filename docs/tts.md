...
OpenMehdi can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere OpenMehdi can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, OpenMehdi defaults
...
TTS config lives under `messages.tts` in `openmehdi.json`.
...
- If `provider` is **unset**, OpenMehdi prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.openmehdi/settings/tts.json`, override with `OPENMEHDI_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, OpenMehdi retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, OpenMehdi:
...
Discord note: `/tts` is a built-in Discord command, so OpenMehdi registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from OpenMehdi
```

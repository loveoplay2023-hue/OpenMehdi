...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.openmehdi/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.openmehdi/node.json` (node id + pairing token).
...
Path: `~/.openmehdi/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.openmehdi/exec-approvals.sock",
...
- Unix socket at `~/.openmehdi/exec-approvals.sock` (0600).

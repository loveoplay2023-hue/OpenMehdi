...
- Publish as `openmehdi/plugin-sdk` (or export from core under `openmehdi/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpenMehdiPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenMehdiConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenMehdiConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenMehdiConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenMehdiConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenMehdiConfig): string;
  };
};
```
...
- Introduce `openmehdi/plugin-sdk`.
- Add `api.runtime` to `OpenMehdiPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openmehdiRuntime: ">=2026.2.0"`).

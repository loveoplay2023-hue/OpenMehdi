...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openmehdi sandbox explain`.
...
Legacy `agent.*` configs are migrated by `openmehdi doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=openmehdi-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENMEHDI_STATE_DIR:-$HOME/.openmehdi}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```

---
summary: "Agent tool surface for OpenMehdi (browser, canvas, nodes, message, cron) replacing legacy `openmehdi-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `openmehdi-*` skills
title: "Tools"
---

# Tools (OpenMehdi)

OpenMehdi exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `openmehdi-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `openmehdi.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenMehdi logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:openmehdi`: all built-in OpenMehdi tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`openmehdi node run`).
...
OpenMehdi tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `openmehdi configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OpenMehdi-managed browser.
...
- Quick smoke: `openmehdi nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `openmehdi gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, OpenMehdi clamps `tools.sessions.visibility` to `tree`.

...
- **gateway host** → `openmehdi` process on the gateway machine
...
`~/.openmehdi/exec-approvals.json` directly.

CLI: `openmehdi approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).

---
summary: "Chrome extension: let OpenMehdi drive your existing Chrome tab"
read_when:
...
The OpenMehdi Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openmehdi-managed Chrome profile.
...
OpenMehdi then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenMehdi release (npm package) as static files. There is no separate “build” step.

After upgrading OpenMehdi:

- Re-run `openmehdi browser extension install` to refresh the installed files under your OpenMehdi state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

OpenMehdi ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want OpenMehdi to control.
...
- `ON`: attached; OpenMehdi can drive that tab.
...
Debugging: `openmehdi sandbox explain`
...
`openmehdi browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `openmehdi browser extension install` first to copy the extension to a stable location under your OpenMehdi state directory.
...
- **This is not isolated** like the dedicated openmehdi-managed profile.

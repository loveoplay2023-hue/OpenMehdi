...
import type { OpenMehdiPluginApi, OpenMehdiPluginService } from "openmehdi/plugin-sdk";
...
function normalizeDenyList(cfg: OpenMehdiPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpenMehdiPluginApi["config"]): string[] {
...
export default function register(api: OpenMehdiPluginApi) {
  ...
  const timerService: OpenMehdiPluginService = {

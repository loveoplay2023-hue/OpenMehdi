...
import { isWSL2Sync } from "openmehdi/plugin-sdk";

const CLIENT_ID_KEYS = ["OPENMEHDI_GEMINI_OAUTH_CLIENT_ID", "GEMINI_CLI_OAUTH_CLIENT_ID"];
const CLIENT_SECRET_KEYS = [
  "OPENMEHDI_GEMINI_OAUTH_CLIENT_SECRET",
...
export function extractGeminiCliCredentials(): { clientId: string; clientSecret: string } | null {
  ...
  try {
    ...
    const secretMatch = content.match(/(GOCSPX-REMOVED[A-Za-z0-9_-]+)/);
      ...
      try {
        ...
        res.end(
          "<!doctype html><html><head><meta charset='utf-8'/></head>" +
            ...
            "<p>You can close this window and return to OpenMehdi.</p></body></html>",
        );
...
async function discoverProject(accessToken: string): Promise<string> {
  ...
  const headers = {
    ...
    "X-Goog-Api-Client": "gl-node/openmehdi",
  };

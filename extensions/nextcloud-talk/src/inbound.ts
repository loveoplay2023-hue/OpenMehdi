...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OpenMehdiConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OpenMehdiConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as OpenMehdiConfig);

...
import { approveDevicePairing, listDevicePairing } from "openmehdi/plugin-sdk";
...
function resolveGatewayPort(cfg: OpenMehdiPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.OPENMEHDI_GATEWAY_PORT?.trim()) ??
...
async function resolveTailnetHost(api: OpenMehdiPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: OpenMehdiPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: OpenMehdiPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the OpenMehdi iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the OpenMehdi iOS app:",
            "",
            "```",
            qrAscii,
            "```",

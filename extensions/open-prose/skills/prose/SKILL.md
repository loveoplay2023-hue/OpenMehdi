...
metadata: { "openmehdi": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OpenMehdi `sessions_spawn`
- **File I/O** == OpenMehdi `read`/`write`
- **Remote fetch** == OpenMehdi `web_fetch` (or `exec` with curl when POST is required)

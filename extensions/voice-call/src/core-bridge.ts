...
function resolveOpenMehdiRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENMEHDI_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPENMEHDI_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpenMehdiRoot(), "dist", "extensionAPI.js");

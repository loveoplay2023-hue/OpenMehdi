...
`openmehdi.invoke --each`) without adding new OpenMehdi code per workflow.
...
This extension depends on OpenMehdi internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenMehdi extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.openmehdi/extensions` as a standalone plugin directory.

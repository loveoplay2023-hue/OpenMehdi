...
OpenMehdi extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
openmehdi channels login --channel zalouser
...
```bash
openmehdi message send --channel zalouser --target <threadId> --message "Hello from OpenMehdi!"
```
...
```bash
openmehdi channels login --channel zalouser              # Login via QR
openmehdi channels login --channel zalouser --account work
openmehdi channels status --probe
openmehdi channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
openmehdi directory self --channel zalouser
openmehdi directory peers list --channel zalouser --query "name"
openmehdi directory groups list --channel zalouser --query "work"
openmehdi directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
openmehdi message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
openmehdi message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `openmehdi channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
openmehdi channels login --channel zalouser --account work
openmehdi message send --channel zalouser --account work --target <id> --message "Hello"

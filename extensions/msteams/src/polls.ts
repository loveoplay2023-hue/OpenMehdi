...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["openmehdiPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["openmehdi", "pollId"]) ??
    readNestedString(value, ["openmehdi", "poll", "id"]) ??
    readNestedString(value, ["data", "openmehdiPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "openmehdi", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          openmehdiPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "openmehdi poll vote",
          displayText: "Vote recorded",
          value: { openmehdiPollId: pollId, pollId },
        },
      },
    ],
  };

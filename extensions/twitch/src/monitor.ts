...
import type { ReplyPayload, OpenMehdiConfig } from "openmehdi/plugin-sdk";
import { createReplyPrefixOptions } from "openmehdi/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // OpenMehdiConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as OpenMehdiConfig;

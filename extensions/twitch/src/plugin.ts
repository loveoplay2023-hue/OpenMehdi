/**
 * Twitch channel plugin for OpenMehdi.
 *
...
import { buildChannelConfigSchema } from "openmehdi/plugin-sdk";
 ...
 * for OpenMehdi. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OpenMehdiConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OpenMehdiConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OpenMehdiConfig): boolean => {

...
const shouldBuild = (deps) => {
  if (deps.env.OPENMEHDI_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OPENMEHDI_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[openmehdi] ${message}\n`);
};

const runOpenMehdi = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["openmehdi.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runOpenMehdi(deps);
  }

 ...
 * `openmehdi update`. Use it to verify the completion installation flow
...
\`openmehdi update\`, \`openmehdi doctor\`, and \`openmehdi onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenMehdi Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`openmehdi completion --install\` later to enable.`));
    return;
  }

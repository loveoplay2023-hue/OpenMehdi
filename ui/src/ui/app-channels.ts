...
export async function handleWhatsAppStart(host: OpenMehdiApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OpenMehdiApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OpenMehdiApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OpenMehdiApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OpenMehdiApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OpenMehdiApp): string {
...
function resolveGatewayHttpAuthHeader(host: OpenMehdiApp): string | null {
...
function buildGatewayHttpHeaders(host: OpenMehdiApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: OpenMehdiApp) {
...
export function handleNostrProfileToggleAdvanced(host: OpenMehdiApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OpenMehdiApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OpenMehdiApp) {
  const state = host.nostrProfileFormState;

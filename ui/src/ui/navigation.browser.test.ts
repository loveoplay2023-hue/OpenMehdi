...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openmehdi/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openmehdi");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openmehdi/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENMEHDI_CONTROL_UI_BASE_PATH__ = "/openmehdi";
    const app = mountApp("/openmehdi/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openmehdi");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openmehdi/sessions");
  });

...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openmehdi-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "openmehdi-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "openmehdi-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "openmehdi-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.openmehdi when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openmehdi-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENMEHDI_HOME over HOME", () => {
    vi.stubEnv("OPENMEHDI_HOME", "/srv/openmehdi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/openmehdi-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENMEHDI_HOME prefix when OPENMEHDI_HOME is set", () => {
    vi.stubEnv("OPENMEHDI_HOME", "/srv/openmehdi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/openmehdi-home")}/.openmehdi/openmehdi.json`)).toBe(
      "$OPENMEHDI_HOME/.openmehdi/openmehdi.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENMEHDI_HOME replacement when OPENMEHDI_HOME is set", () => {
    vi.stubEnv("OPENMEHDI_HOME", "/srv/openmehdi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/openmehdi-home")}/.openmehdi/openmehdi.json`),
    ).toBe("config: $OPENMEHDI_HOME/.openmehdi/openmehdi.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/openmehdi")).toBe(path.resolve(os.homedir(), "openmehdi"));
  });
  ...
  it("prefers OPENMEHDI_HOME for tilde expansion", () => {
    vi.stubEnv("OPENMEHDI_HOME", "/srv/openmehdi-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/openmehdi")).toBe(path.resolve("/srv/openmehdi-home", "openmehdi"));

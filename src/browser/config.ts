import type { BrowserConfig, BrowserProfileConfig, OpenMehdiConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENMEHDI_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "openmehdi" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_OPENMEHDI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openmehdi" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENMEHDI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENMEHDI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenMehdi browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openmehdi/openmehdi CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenMehdiConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENMEHDI_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openmehdi";

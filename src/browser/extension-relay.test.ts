...
describe("chrome extension relay server", () => {
  ...
  it("derives relay auth headers from gateway token for loopback URLs", async () => {
    ...
    process.env.OPENMEHDI_GATEWAY_TOKEN = "test-gateway-token";
    try {
      const headers = getChromeExtensionRelayAuthHeaders(`http://127.0.0.1:${port}`);
      expect(Object.keys(headers)).toContain("x-openmehdi-relay-token");
      expect((headers["x-openmehdi-relay-token"] ?? "").length).toBeGreaterThan(20);
    } finally {
      if (prev === undefined) {
        delete process.env.OPENMEHDI_GATEWAY_TOKEN;
      } else {
        process.env.OPENMEHDI_GATEWAY_TOKEN = prev;
      }
    }
  });
  ...
  it("does not swallow EADDRINUSE when occupied port is not an openmehdi relay", async () => {
    const port = await getFreePort();

...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const openmehdi = resolveProfile(resolved, "openmehdi");
    expect(openmehdi?.driver).toBe("openmehdi");
    expect(openmehdi?.cdpPort).toBe(18800);
    expect(openmehdi?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from OPENMEHDI_GATEWAY_PORT when unset", () => {
    const prev = process.env.OPENMEHDI_GATEWAY_PORT;
    process.env.OPENMEHDI_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const openmehdi = resolveProfile(resolved, "openmehdi");
      expect(openmehdi?.cdpPort).toBe(19012);
      expect(openmehdi?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.OPENMEHDI_GATEWAY_PORT;
      } else {
        process.env.OPENMEHDI_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const openmehdi = resolveProfile(resolved, "openmehdi");
      expect(openmehdi?.cdpPort).toBe(19022);
      expect(openmehdi?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.OPENMEHDI_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "openmehdi");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        openmehdi: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("openmehdi");
  });

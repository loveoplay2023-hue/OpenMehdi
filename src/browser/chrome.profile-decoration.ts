...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openmehdi-profile-decorated");
}
 ...
 */
export function decorateOpenMehdiProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENMEHDI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENMEHDI_BROWSER_COLOR).toUpperCase();

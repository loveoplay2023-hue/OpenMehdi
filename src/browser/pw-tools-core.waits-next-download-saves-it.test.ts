...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredOpenMehdiTmpDir: vi.fn(() => "/tmp/openmehdi"),
}));
vi.mock("../infra/tmp-openmehdi-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredOpenMehdiTmpDir.mockReturnValue("/tmp/openmehdi");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredOpenMehdiTmpDir.mockReturnValue("/tmp/openmehdi-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "openmehdi-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredOpenMehdiTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "openmehdi-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "openmehdi-preferred", "downloads")}${path.sep}`),
    );
  });

...
const RELAY_AUTH_HEADER = "x-openmehdi-relay-token";
...
function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.OPENMEHDI_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
async function looksLikeOpenMehdiRelay(baseUrl: string): Promise<boolean> {
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/OpenMehdi-Extension-Relay",
          revision: "0",
          userAgent: "OpenMehdi-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "OpenMehdi/extension-relay",
  ...
  try {
    ...
    if (isAddrInUseError(err) && (await looksLikeOpenMehdiRelay(info.baseUrl))) {

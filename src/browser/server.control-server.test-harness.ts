...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/openmehdi" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "openmehdi",
        profiles: {
          openmehdi: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchOpenMehdiChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveOpenMehdiUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopOpenMehdiChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.OPENMEHDI_GATEWAY_PORT;
    process.env.OPENMEHDI_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.OPENMEHDI_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.OPENMEHDI_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.OPENMEHDI_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.OPENMEHDI_GATEWAY_TOKEN;
    } else {
      process.env.OPENMEHDI_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.OPENMEHDI_GATEWAY_PASSWORD;
    } else {
      process.env.OPENMEHDI_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }

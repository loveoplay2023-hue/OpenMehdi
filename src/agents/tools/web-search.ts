...
type WebSearchConfig = NonNullable<OpenMehdiConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OpenMehdiConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("openmehdi configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.openmehdi.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://openmehdi.ai",
      "X-Title": "OpenMehdi Web Search",
    },

...
function resolveDefaultModelRef(cfg?: OpenMehdiConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: OpenMehdiConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: OpenMehdiConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureOpenMehdiModelsJson(effectiveCfg, params.agentDir);

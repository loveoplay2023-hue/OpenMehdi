...
import { createOpenMehdiTools } from "./openmehdi-tools.js";
import { __testing, createOpenMehdiCodingTools } from "./pi-tools.js";
import { createOpenMehdiReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
import { createHostSandboxFsBridge } from "./test-helpers/host-sandbox-fs-bridge.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createOpenMehdiCodingTools();
...
describe("createOpenMehdiCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createOpenMehdiTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpenMehdiCodingTools({ messageProvider: "discord" });
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createOpenMehdiCodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "openmehdi-outside.txt");
  ...
  it("auto-pages read output across chunks when context window budget allows", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-read-autopage-"));
  ...
  it("adds capped continuation guidance when aggregated read output reaches budget", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-read-cap-"));
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createOpenMehdiReadTool(
      baseRead as unknown as Parameters<typeof createOpenMehdiReadTool>[0],
    );

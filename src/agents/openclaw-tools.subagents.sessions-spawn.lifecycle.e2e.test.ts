...
} from "./openmehdi-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateOpenMehdiTools = (typeof import("./openmehdi-tools.js"))["createOpenMehdiTools"];
type CreateOpenMehdiToolsOpts = Parameters<CreateOpenMehdiTools>[0];

async function getSessionsSpawnTool(opts: CreateOpenMehdiToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createOpenMehdiTools } = await import("./openmehdi-tools.js");
  const tool = createOpenMehdiTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("openmehdi-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });

...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("openmehdi-ws-", async (workspaceDir) => {
      await withTempDir("openmehdi-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOpenMehdiCodingTools({ workspaceDir });
    ...
    await withTempDir("openmehdi-ws-", async (workspaceDir) => {
      await withTempDir("openmehdi-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello openmehdi");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("openmehdi-ws-", async (workspaceDir) => {
      await withTempDir("openmehdi-override-", async (overrideDir) => {
        const tools = createOpenMehdiCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("openmehdi-sandbox-", async (sandboxDir) => {
      await withTempDir("openmehdi-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          containerName: "openmehdi-sbx-test",
        ...
        const tools = createOpenMehdiCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");

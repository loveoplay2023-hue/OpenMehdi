...
const originalOpenMehdiStateDir = process.env.OPENMEHDI_STATE_DIR;

afterEach(() => {
  if (originalOpenMehdiStateDir === undefined) {
    delete process.env.OPENMEHDI_STATE_DIR;
  } else {
    process.env.OPENMEHDI_STATE_DIR = originalOpenMehdiStateDir;
  }
});

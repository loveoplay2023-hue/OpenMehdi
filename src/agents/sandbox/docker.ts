...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openmehdi.sandbox=1");
  args.push("--label", `openmehdi.sessionKey=${params.scopeKey}`);
  args.push("--label", `openmehdi.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openmehdi.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "openmehdi.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openmehdi sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openmehdi sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openmehdi sandbox recreate --all");
}

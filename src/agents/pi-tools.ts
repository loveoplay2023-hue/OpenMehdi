...
function resolveExecConfig(params: { cfg?: OpenMehdiConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: OpenMehdiConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createOpenMehdiCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createOpenMehdiReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });

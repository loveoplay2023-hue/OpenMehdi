...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OPENMEHDI_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".openmehdi", `workspace-${profile}`);
  }
  return path.join(home, ".openmehdi", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".openmehdi";

...
async function withTempWorkspace(
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-skills-install-"));
  try {
...
function restoreOpenMehdiStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.OPENMEHDI_STATE_DIR;
    return;
  }
  process.env.OPENMEHDI_STATE_DIR = originalValue;
}

const originalStateDir = process.env.OPENMEHDI_STATE_DIR;

afterEach(() => {
  restoreOpenMehdiStateDir(originalStateDir);
});

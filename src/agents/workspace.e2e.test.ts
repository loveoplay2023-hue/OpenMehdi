...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPENMEHDI_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPENMEHDI_HOME: "/srv/openmehdi-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/openmehdi-home"), ".openmehdi", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".openmehdi", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("openmehdi-workspace-");

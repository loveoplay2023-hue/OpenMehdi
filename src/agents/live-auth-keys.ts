...
const GOOGLE_LIVE_SINGLE_KEY = "OPENMEHDI_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OPENMEHDI_LIVE_ANTHROPIC_KEY",
    listVar: "OPENMEHDI_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OPENMEHDI_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OPENMEHDI_LIVE_${base}_KEY`;

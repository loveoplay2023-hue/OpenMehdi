...
vi.mock("./openmehdi-root.js", () => ({
  resolveOpenMehdiPackageRoot: vi.fn(async () => null),
  resolveOpenMehdiPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveOpenMehdiPackageRoot when available", async () => {
    const openmehdiRoot = await import("./openmehdi-root.js");
    const { resolveControlUiDistIndexPath } = await import("./control-ui-assets.js");

    const pkgRoot = abs("fixtures/openmehdi");
    (
      openmehdiRoot.resolveOpenMehdiPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/openmehdi"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "openmehdi" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "openmehdi.mjs"))).resolves.toBe(
  ...
  it("returns null when fallback package name does not match", async () => {
    ...
    const root = abs("fixtures/not-openmehdi");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    const pkgRoot = abs("fixtures/openmehdi-bundle");
    (
      openmehdiRoot.resolveOpenMehdiPackageRootSync as unknown as ReturnType<typeof vi.fn>

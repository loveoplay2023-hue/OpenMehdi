...
describe("update-startup", () => {
  ...
  let resolveOpenMehdiPackageRoot: (typeof import("./openmehdi-root.js"))["resolveOpenMehdiPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "OPENMEHDI_STATE_DIR");
    prevStateDir = process.env.OPENMEHDI_STATE_DIR;
    process.env.OPENMEHDI_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOpenMehdiPackageRoot } = await import("./openmehdi-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveOpenMehdiPackageRoot).mockResolvedValue("/opt/openmehdi");

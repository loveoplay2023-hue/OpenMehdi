...
import { resolveOpenMehdiPackageRoot, resolveOpenMehdiPackageRootSync } from "./openmehdi-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveOpenMehdiPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "openmehdi" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveOpenMehdiPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // openmehdi.mjs or dist/<bundle>.js

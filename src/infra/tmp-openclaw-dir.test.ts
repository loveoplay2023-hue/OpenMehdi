...
import { POSIX_OPENMEHDI_TMP_DIR, resolvePreferredOpenMehdiTmpDir } from "./tmp-openmehdi-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOpenMehdiTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `openmehdi-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOpenMehdiTmpDir({
    accessSync,
...
describe("resolvePreferredOpenMehdiTmpDir", () => {
  it("prefers /tmp/openmehdi when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_OPENMEHDI_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/openmehdi when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OPENMEHDI_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/openmehdi when /tmp/openmehdi is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/openmehdi when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/openmehdi is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/openmehdi is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/openmehdi is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,

...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "openmehdi-exec-approvals-"));
}
...
describe("resolve exec approvals defaults", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    const prevOpenMehdiHome = process.env.OPENMEHDI_HOME;
    try {
      process.env.OPENMEHDI_HOME = dir;
      expect(path.normalize(resolveExecApprovalsPath())).toBe(
        path.normalize(path.join(dir, ".openmehdi", "exec-approvals.json")),
      );
      expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
        path.normalize(path.join(dir, ".openmehdi", "exec-approvals.sock")),
      );
    } finally {
      if (prevOpenMehdiHome === undefined) {
        delete process.env.OPENMEHDI_HOME;
      } else {
        process.env.OPENMEHDI_HOME = prevOpenMehdiHome;
      }
    }
  });
});
...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    try {
      process.env.OPENMEHDI_HOME = dir;
      const approvalsPath = path.join(dir, ".openmehdi", "exec-approvals.json");

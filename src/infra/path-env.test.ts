...
let ensureOpenMehdiCliOnPath: typeof import("./path-env.js").ensureOpenMehdiCliOnPath;

describe("ensureOpenMehdiCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OPENMEHDI_PATH_BOOTSTRAPPED",
    "OPENMEHDI_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOpenMehdiCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling openmehdi exists", () => {
    const tmp = abs("/tmp/openmehdi-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "openmehdi");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.OPENMEHDI_PATH_BOOTSTRAPPED;

    ensureOpenMehdiCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.OPENMEHDI_PATH_BOOTSTRAPPED = "1";
    ensureOpenMehdiCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/openmehdi-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openmehdi");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/openmehdi-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openmehdi");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "openmehdi");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/openmehdi-path/case-linuxbrew");

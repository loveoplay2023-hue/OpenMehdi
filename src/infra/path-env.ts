...
function candidateBinDirs(opts: EnsureOpenMehdiPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `openmehdi` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openmehdi");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OPENMEHDI_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "openmehdi"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openmehdi` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenMehdiCliOnPath(opts: EnsureOpenMehdiPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENMEHDI_PATH_BOOTSTRAPPED)) {
    return;
  }

...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.OPENMEHDI_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("openmehdi-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("openmehdi (OpenMehdi)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("openmehdi");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("openmehdi.local");

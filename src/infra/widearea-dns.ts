...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENMEHDI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openmehdi-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openmehdi";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openmehdi");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openmehdi-gw");
  ...
  records.push(`_openmehdi-gw._tcp IN PTR ${instanceLabel}._openmehdi-gw._tcp`);
  records.push(`${instanceLabel}._openmehdi-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openmehdi-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openmehdi-content-hash: ${contentHash}\n${contentBody}`;
}

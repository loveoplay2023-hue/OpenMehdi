...
const DEFAULT_PACKAGE_NAME = "openmehdi";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "openmehdi.mjs");
    ...
    const doctorStep = await runStep(
      step("openmehdi doctor", doctorArgv, gitRoot, { OPENMEHDI_UPDATE_IN_PROGRESS: "1" }),
    );

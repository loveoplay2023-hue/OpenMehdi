...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.OPENMEHDI_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.OPENMEHDI_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.OPENMEHDI_TEST_TAILSCALE_BINARY;
    } else {
      process.env.OPENMEHDI_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });

...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OpenMehdiConfig | null = null;
  ...
  get(key: string, cfg: OpenMehdiConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OpenMehdiConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: OpenMehdiConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: OpenMehdiConfig): void {

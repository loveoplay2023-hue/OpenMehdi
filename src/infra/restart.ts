...
export function triggerOpenMehdiRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENMEHDI_SYSTEMD_UNIT,
        process.env.OPENMEHDI_PROFILE,
      );
  ...
  const label =
    process.env.OPENMEHDI_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENMEHDI_PROFILE);

...
import type { OpenMehdiConfig } from "../config/types.js";
import { resolvePreferredOpenMehdiTmpDir } from "../infra/tmp-openmehdi-dir.js";
import { readLoggingConfig } from "./config.js";
import type { ConsoleStyle } from "./console.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredOpenMehdiTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "openmehdi.log"); // legacy single-file path

const LOG_PREFIX = "openmehdi";
...
function resolveSettings(): ResolvedSettings {
  let cfg: OpenMehdiConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => OpenMehdiConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.OPENMEHDI_TEST_FILE_LOG !== "1" ? "silent" : "info";

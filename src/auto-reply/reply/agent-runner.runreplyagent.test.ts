...
beforeEach(() => {
  ...
  vi.stubEnv("OPENMEHDI_TEST_FAST", "1");
});
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { OPENMEHDI_STATE_DIR: process.env.OPENMEHDI_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.OPENMEHDI_STATE_DIR === undefined) {
      delete process.env.OPENMEHDI_STATE_DIR;
    } else {
      process.env.OPENMEHDI_STATE_DIR = snapshot.OPENMEHDI_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "openmehdi-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "openmehdi-memory-flush-"));
  });

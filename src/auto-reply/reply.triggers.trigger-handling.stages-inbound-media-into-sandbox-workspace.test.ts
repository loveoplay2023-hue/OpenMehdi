...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("openmehdi-triggers-", async (home) => {
      const inboundDir = join(home, ".openmehdi", "media", "inbound");
      ...
      await stageSandboxMedia({
        ...
        workspaceDir: join(home, "openmehdi"),
      });
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("openmehdi-triggers-bypass-", async (home) => {
      // Sensitive host file outside .openmehdi

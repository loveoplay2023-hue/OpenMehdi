...
export function isCommandEnabled(cfg: OpenMehdiConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenMehdiConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenMehdiConfig),
...
export function getCommandDetection(_cfg?: OpenMehdiConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenMehdiConfig) {

...
describe("shared/frontmatter", () => {
  ...
  test("resolveOpenMehdiManifestBlock parses JSON5 metadata and picks openmehdi block", () => {
    const frontmatter = {
      metadata: "{ openmehdi: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveOpenMehdiManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveOpenMehdiManifestBlock returns undefined for invalid input", () => {
    expect(resolveOpenMehdiManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveOpenMehdiManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveOpenMehdiManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});

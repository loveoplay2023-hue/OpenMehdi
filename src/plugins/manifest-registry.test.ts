...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openmehdi-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "openmehdi.plugin.json"), JSON.stringify(manifest), "utf-8");
}

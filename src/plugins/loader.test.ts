...
import { loadOpenMehdiPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `openmehdi-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.OPENMEHDI_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openmehdi.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.OPENMEHDI_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadOpenMehdiPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.OPENMEHDI_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOpenMehdiPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadOpenMehdiPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOpenMehdiPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],

...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["OPENMEHDI_BASE_ENV"]);
    process.env.OPENMEHDI_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.OPENMEHDI_BASE_ENV ?? "") + "|" + (process.env.OPENMEHDI_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { OPENMEHDI_TEST_ENV: "ok" },
        },
      );

...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".openmehdi");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENMEHDI_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENMEHDI_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENMEHDI_STATE_DIR.

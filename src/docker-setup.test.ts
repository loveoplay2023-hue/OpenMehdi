...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "openmehdi-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    OPENMEHDI_GATEWAY_TOKEN: "test-token",
    OPENMEHDI_CONFIG_DIR: join(sandbox.rootDir, "config"),
    OPENMEHDI_WORKSPACE_DIR: join(sandbox.rootDir, "openmehdi"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        OPENMEHDI_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        OPENMEHDI_EXTRA_MOUNTS: undefined,
        OPENMEHDI_HOME_VOLUME: "openmehdi-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENMEHDI_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("OPENMEHDI_EXTRA_MOUNTS=");
    expect(envFile).toContain("OPENMEHDI_HOME_VOLUME=openmehdi-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("openmehdi-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("openmehdi-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg OPENMEHDI_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });

...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    OPENMEHDI_STATE_DIR: process.env.OPENMEHDI_STATE_DIR,
    OPENMEHDI_CONFIG_PATH: process.env.OPENMEHDI_CONFIG_PATH,
    OPENMEHDI_GATEWAY_PORT: process.env.OPENMEHDI_GATEWAY_PORT,
    OPENMEHDI_PROFILE: process.env.OPENMEHDI_PROFILE,
  };

  beforeEach(() => {
    process.env.OPENMEHDI_STATE_DIR = "/tmp/openmehdi-cli-state";
    process.env.OPENMEHDI_CONFIG_PATH = "/tmp/openmehdi-cli-state/openmehdi.json";
    delete process.env.OPENMEHDI_GATEWAY_PORT;
    delete process.env.OPENMEHDI_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.OPENMEHDI_STATE_DIR !== undefined) {
      process.env.OPENMEHDI_STATE_DIR = originalEnv.OPENMEHDI_STATE_DIR;
    } else {
      delete process.env.OPENMEHDI_STATE_DIR;
    }

    if (originalEnv.OPENMEHDI_CONFIG_PATH !== undefined) {
      process.env.OPENMEHDI_CONFIG_PATH = originalEnv.OPENMEHDI_CONFIG_PATH;
    } else {
      delete process.env.OPENMEHDI_CONFIG_PATH;
    }

    if (originalEnv.OPENMEHDI_GATEWAY_PORT !== undefined) {
      process.env.OPENMEHDI_GATEWAY_PORT = originalEnv.OPENMEHDI_GATEWAY_PORT;
    } else {
      delete process.env.OPENMEHDI_GATEWAY_PORT;
    }

    if (originalEnv.OPENMEHDI_PROFILE !== undefined) {
      process.env.OPENMEHDI_PROFILE = originalEnv.OPENMEHDI_PROFILE;
    } else {
      delete process.env.OPENMEHDI_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OPENMEHDI_PROFILE: "dev",
        OPENMEHDI_STATE_DIR: "/tmp/openmehdi-daemon-state",
        OPENMEHDI_CONFIG_PATH: "/tmp/openmehdi-daemon-state/openmehdi.json",
        OPENMEHDI_GATEWAY_PORT: "19001",
      },

    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openmehdi memory status", "Show index and provider status."],
          ["openmehdi memory index --force", "Force a full reindex."],
          ['openmehdi memory search --query "deployment notes"', "Search indexed memory entries."],
          ["openmehdi memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.openmehdi.ai/cli/memory")}\n`,
    );

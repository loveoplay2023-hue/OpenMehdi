...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.openmehdi.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + OpenMehdi hooks")
    ...
    .option("--hook-url <url>", "OpenMehdi hook URL")
    .option("--hook-token <token>", "OpenMehdi hook token")

...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openmehdi-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPENMEHDI_NO_RESPAWN: "1",
      OPENMEHDI_STATE_DIR: stateDir,
      OPENMEHDI_SKIP_CHANNELS: "1",
      OPENMEHDI_SKIP_GMAIL_WATCHER: "1",
      OPENMEHDI_SKIP_CRON: "1",
      OPENMEHDI_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPENMEHDI_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "openmehdi-entry-bootstrap.mjs");

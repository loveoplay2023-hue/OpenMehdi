    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "openmehdi node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["openmehdi node status", "Check node host service status."],
          ["openmehdi node install", "Install the node host service."],
          ["openmehdi node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.openmehdi.ai/cli/node")}\n`,
    );

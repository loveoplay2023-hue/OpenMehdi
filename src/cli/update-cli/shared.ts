...
import { resolveOpenMehdiPackageRoot } from "../../infra/openmehdi-root.js";
...
const OPENMEHDI_REPO_URL = "https://github.com/openmehdi/openmehdi.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "openmehdi";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("openmehdi@")) {
    return trimmed.slice("openmehdi@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.OPENMEHDI_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveOpenMehdiPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OPENMEHDI_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENMEHDI_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENMEHDI_GIT_DIR to an empty folder or an openmehdi checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPENMEHDI_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "openmehdi.mjs");

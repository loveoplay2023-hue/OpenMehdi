...
${formatHelpExamples([
  ['openmehdi agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openmehdi agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openmehdi agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openmehdi agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openmehdi agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openmehdi agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openmehdi.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openmehdi.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openmehdi agents set-identity --agent main --name "OpenMehdi" --emoji "🦞"', "Set name + emoji."],
  ["openmehdi agents set-identity --agent main --avatar avatars/openmehdi.png", "Set avatar path."],
  [
    "openmehdi agents set-identity --workspace ~/.openmehdi/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "openmehdi agents set-identity --identity-file ~/.openmehdi/workspace/IDENTITY.md --agent main",

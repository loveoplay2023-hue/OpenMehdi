...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.OPENMEHDI_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.OPENMEHDI_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<OpenMehdiConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live OpenMehdi docs",
    hasSubcommands: false,
    register: async (program) => {

    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openmehdi nodes status", "List known nodes with live status."],
          ["openmehdi nodes pairing pending", "Show pending node pairing requests."],
          ['openmehdi nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["openmehdi nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.openmehdi.ai/cli/nodes")}\n`,
    );

...
import type { OpenMehdiConfig, ConfigFileSnapshot } from "../config/types.openmehdi.js";
...
vi.mock("../infra/openmehdi-root.js", () => ({
  resolveOpenMehdiPackageRoot: vi.fn(),
}));
...
const { resolveOpenMehdiPackageRoot } = await import("../infra/openmehdi-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-update-tests-"));
  });
  ...
  const baseConfig = {} as OpenMehdiConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/openmehdi-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOpenMehdiPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("openmehdi-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveOpenMehdiPackageRoot).mockReset();
    ...
    vi.mocked(resolveOpenMehdiPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("openmehdi");
    readPackageVersion.mockResolvedValue("1.0.0");
    resolveGlobalManager.mockResolvedValue("npm");
    serviceLoaded.mockResolvedValue(false);
    prepareRestartScript.mockResolvedValue("/tmp/openmehdi-restart-test.sh");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("OpenMehdi update status");
  });
  ...
  it("uses stored beta channel when configured", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...baseSnapshot,
      config: { update: { channel: "beta" } } as OpenMehdiConfig,
    });
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "git",
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveOpenMehdiPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "npm",
      steps: [],
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    const envSnapshot = captureEnv(["OPENMEHDI_UPDATE_IN_PROGRESS"]);
    const randomSpy = vi.spyOn(Math, "random").mockReturnValue(0);
    try {
      delete process.env.OPENMEHDI_UPDATE_IN_PROGRESS;
      vi.mocked(runGatewayUpdate).mockResolvedValue(mockResult);
      vi.mocked(runDaemonRestart).mockResolvedValue(true);
      ...
      expect(process.env.OPENMEHDI_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("openmehdi-update-wizard");
    const envSnapshot = captureEnv(["OPENMEHDI_GIT_DIR"]);
    try {
      setTty(true);
      process.env.OPENMEHDI_GIT_DIR = tempDir;

...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["OPENMEHDI_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "openmehdi-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "openmehdi-bundled-skills-test-"));
    process.env.OPENMEHDI_BUNDLED_SKILLS_DIR = tempBundledDir;
  });

...
import type { ConfigFileSnapshot, OpenMehdiConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `openmehdi config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<(cfg: OpenMehdiConfig) => Promise<void>>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: OpenMehdiConfig) => mockWriteConfigFile(cfg),
}));
...
function setSnapshot(resolved: OpenMehdiConfig, config: OpenMehdiConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      const resolved: OpenMehdiConfig = {
        agents: {
      ...
      const runtimeMerged: OpenMehdiConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {

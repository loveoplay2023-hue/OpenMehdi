...
function listAgents(cfg: OpenMehdiConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: OpenMehdiConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OpenMehdiConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<OpenMehdiConfig, EvaluatedBindingsCache>();

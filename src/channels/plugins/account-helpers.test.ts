...
function cfg(accounts?: Record<string, unknown> | null): OpenMehdiConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as OpenMehdiConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as OpenMehdiConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as OpenMehdiConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as OpenMehdiConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as OpenMehdiConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as OpenMehdiConfig)).toBe("default");
    });
  });
});

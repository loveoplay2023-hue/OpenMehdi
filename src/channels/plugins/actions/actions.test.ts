...
function telegramCfg(): OpenMehdiConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OpenMehdiConfig;
}

function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as OpenMehdiConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as OpenMehdiConfig;

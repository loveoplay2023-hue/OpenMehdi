...
import { discoverOpenMehdiPlugins } from "../../plugins/discovery.js";
import type { OpenMehdiPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, OpenMehdiPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["OPENMEHDI_PLUGIN_CATALOG_PATHS", "OPENMEHDI_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<OpenMehdiPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: OpenMehdiPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: OpenMehdiPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverOpenMehdiPlugins({ workspaceDir: options.workspaceDir });

...
function summarizeGroupPolicy(cfg: OpenMehdiConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: OpenMehdiConfig): boolean {
...
function collectModels(cfg: OpenMehdiConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: OpenMehdiConfig): Set<string> {
  const baseCfg: OpenMehdiConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: OpenMehdiConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OpenMehdiConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OpenMehdiConfig): boolean {
...
function isBrowserEnabled(cfg: OpenMehdiConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: OpenMehdiConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: OpenMehdiConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep OPENMEHDI_STATE_DIR on a local-only volume and re-run "${formatCliCommand("openmehdi security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: OpenMehdiConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer OPENMEHDI_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const openmehdiGatewayToken =
    typeof env.OPENMEHDI_GATEWAY_TOKEN === "string" && env.OPENMEHDI_GATEWAY_TOKEN.trim()
      ? env.OPENMEHDI_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-openmehdi-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: OpenMehdiConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: OpenMehdiConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: OpenMehdiConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: OpenMehdiConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: OpenMehdiConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: OpenMehdiConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];

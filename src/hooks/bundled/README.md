...
This directory contains hooks that ship with OpenMehdi. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.openmehdi/openmehdi.json`:
...
2. Restart gateway: `pkill -9 -f 'openmehdi.*gateway' && pnpm openmehdi gateway`
3. Enable the hook: `openmehdi hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.openmehdi.ai/automation/hooks

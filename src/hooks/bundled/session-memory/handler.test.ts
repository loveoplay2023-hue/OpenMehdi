...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => OpenMehdiConfig;
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("openmehdi-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): OpenMehdiConfig {
  return {
    agents: { defaults: { workspace: tempDir } },

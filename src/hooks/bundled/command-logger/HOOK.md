...
homepage: https://docs.openmehdi.ai/automation/hooks#command-logger
metadata:
  {
    "openmehdi":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.openmehdi/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.openmehdi/logs/commands.log ~/.openmehdi/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openmehdi`:
   ```
   /home/username/.openmehdi/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok

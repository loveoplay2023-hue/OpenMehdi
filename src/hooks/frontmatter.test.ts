...
`;
    ...
    expect(parsed.openmehdi.emoji).toBe("💾");
    expect(parsed.openmehdi.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.openmehdi.emoji).toBe("📝");
    expect(parsed.openmehdi.events).toEqual(["command"]);
    expect(parsed.openmehdi.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.openmehdi.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"openmehdi": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"openmehdi": {"events": ["test"]}}');
  });
...
describe("resolveOpenMehdiMetadata", () => {
  it("extracts openmehdi metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenMehdiMetadata(frontmatter);
  ...
  it("returns undefined when openmehdi key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        openmehdi: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenMehdi" },
            { id: "npm", kind: "npm", package: "@openmehdi/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@openmehdi/hook");
  });
...
`;
    ...
    const openmehdi = resolveOpenMehdiMetadata(frontmatter);
    expect(openmehdi).toBeDefined();
    expect(openmehdi?.emoji).toBe("💾");
    expect(openmehdi?.events).toEqual(["command:new"]);
    expect(openmehdi?.requires?.config).toEqual(["workspace.dir"]);
    expect(openmehdi?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const openmehdi = resolveOpenMehdiMetadata(frontmatter);
    expect(openmehdi?.emoji).toBe("disk");
    expect(openmehdi?.events).toEqual(["command:new"]);
  });
});

...
type GetReplyFromConfigFn = (
  ...
  configOverride?: OpenMehdiConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `openmehdi-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.OPENMEHDI_CONFIG_PATH = path.join(root, "openmehdi.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "openmehdi.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "openmehdi-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadOpenMehdiPlugins: () => pluginRegistryState.registry,
  };
});

...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.OPENMEHDI_STATE_DIR;
  previousConfigPath = process.env.OPENMEHDI_CONFIG_PATH;
  previousSkipBrowserControl = process.env.OPENMEHDI_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.OPENMEHDI_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.OPENMEHDI_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.OPENMEHDI_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.OPENMEHDI_SKIP_CHANNELS;
  previousSkipProviders = process.env.OPENMEHDI_SKIP_PROVIDERS;
  previousSkipCron = process.env.OPENMEHDI_SKIP_CRON;
  previousMinimalGateway = process.env.OPENMEHDI_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENMEHDI_STATE_DIR = path.join(tempHome, ".openmehdi");
  delete process.env.OPENMEHDI_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OPENMEHDI_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPENMEHDI_SKIP_GMAIL_WATCHER = "1";
  process.env.OPENMEHDI_SKIP_CANVAS_HOST = "1";
  process.env.OPENMEHDI_SKIP_CHANNELS = "1";
  process.env.OPENMEHDI_SKIP_PROVIDERS = "1";
  process.env.OPENMEHDI_SKIP_CRON = "1";
  process.env.OPENMEHDI_TEST_MINIMAL_GATEWAY = "1";
  process.env.OPENMEHDI_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "openmehdi-test-no-bundled-extensions")
    : "openmehdi-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "openmehdi-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".openmehdi-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      delete process.env.OPENMEHDI_STATE_DIR;
    } else {
      process.env.OPENMEHDI_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.OPENMEHDI_CONFIG_PATH;
    } else {
      process.env.OPENMEHDI_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.OPENMEHDI_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.OPENMEHDI_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.OPENMEHDI_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENMEHDI_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.OPENMEHDI_SKIP_CANVAS_HOST;
    } else {
      process.env.OPENMEHDI_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.OPENMEHDI_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.OPENMEHDI_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.OPENMEHDI_SKIP_CHANNELS;
    } else {
      process.env.OPENMEHDI_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.OPENMEHDI_SKIP_PROVIDERS;
    } else {
      process.env.OPENMEHDI_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.OPENMEHDI_SKIP_CRON;
    } else {
      process.env.OPENMEHDI_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.OPENMEHDI_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.OPENMEHDI_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["OPENMEHDI_GATEWAY_TOKEN"]);
  const prev = process.env.OPENMEHDI_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    delete process.env.OPENMEHDI_GATEWAY_TOKEN;
  } else {
    process.env.OPENMEHDI_GATEWAY_TOKEN = fallbackToken;
  }

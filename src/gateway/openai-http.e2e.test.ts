    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "openmehdi", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-openmehdi-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-openmehdi-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "openmehdi", messages: [{ role: "user", content: "hi" }] },
          {
            "x-openmehdi-agent-id": "beta",
            "x-openmehdi-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);

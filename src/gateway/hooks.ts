...
export function resolveHooksConfig(cfg: OpenMehdiConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OpenMehdiConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-openmehdi-token"] === "string"
      ? req.headers["x-openmehdi-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }

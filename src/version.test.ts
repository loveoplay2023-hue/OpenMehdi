...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({ name: "openmehdi", version: "1.2.3" }),
        "utf-8",
      );
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      ...
      await fs.writeFile(
        path.join(root, "package.json"),
        JSON.stringify({ name: "openmehdi", version: "2.3.4" }),
        "utf-8",
      );
      await fs.writeFile(
  ...
  it("prefers OPENMEHDI_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        OPENMEHDI_VERSION: "9.9.9",
        OPENMEHDI_SERVICE_VERSION: "2.2.2",
  ...
  it("uses service and package fallbacks and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        OPENMEHDI_VERSION: "   ",
        OPENMEHDI_SERVICE_VERSION: "  2.0.0  ",

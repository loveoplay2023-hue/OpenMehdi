...
export async function loadValidConfigOrThrow(): Promise<OpenMehdiConfig> {
...
export async function updateConfig(
  mutator: (cfg: OpenMehdiConfig) => OpenMehdiConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenMehdiConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenMehdiConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openmehdi agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}

...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenMehdiAgentDir: vi.fn().mockReturnValue("/tmp/openmehdi-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openmehdi-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/openmehdi-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenMehdiAgentDir: mocks.resolveOpenMehdiAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/openmehdi-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenMehdiAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/openmehdi-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/openmehdi-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/openmehdi-agent-custom");

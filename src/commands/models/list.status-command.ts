...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenMehdiAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openmehdi models auth setup-token")}\` or \`${formatCliCommand("openmehdi configure")}\`.`
          : `Run \`${formatCliCommand("openmehdi configure")}\` or set an API key env var.`;

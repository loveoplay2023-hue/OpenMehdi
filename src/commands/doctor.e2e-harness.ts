...
export const resolveOpenMehdiPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenMehdiPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/openmehdi.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveOpenMehdiPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENMEHDI_STATE_DIR;
  originalUpdateInProgress = process.env.OPENMEHDI_UPDATE_IN_PROGRESS;
  process.env.OPENMEHDI_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openmehdi-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENMEHDI_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENMEHDI_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENMEHDI_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {

...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: OpenMehdiConfig },
  primary: string,
) {
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: OpenMehdiConfig) => config,
      applyDefaultConfig: (config: OpenMehdiConfig) => config,
      noteAgentModel,
...
describe("applyGoogleGeminiModelDefault", () => {
  it("sets gemini default when model is unset", () => {
    const cfg: OpenMehdiConfig = { agents: { defaults: {} } };

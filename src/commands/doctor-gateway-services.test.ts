...
function setupGatewayTokenRepairScenario(expectedToken: string) {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      OPENMEHDI_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service OPENMEHDI_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
  mocks.buildGatewayInstallPlan.mockResolvedValue({
    ...
    environment: {
      OPENMEHDI_GATEWAY_TOKEN: expectedToken,
    },
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  ...
  it("uses OPENMEHDI_GATEWAY_TOKEN when config token is missing", async () => {
    const previousToken = process.env.OPENMEHDI_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousToken === undefined) {
        ...
        process.env.OPENMEHDI_GATEWAY_TOKEN = previousToken;
      }
    }
  });
});

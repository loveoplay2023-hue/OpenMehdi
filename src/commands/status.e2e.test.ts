...
beforeAll(() => {
  envSnapshot = captureEnv(["OPENMEHDI_PROFILE"]);
  process.env.OPENMEHDI_PROFILE = "isolated";
});
...
vi.mock("../infra/openmehdi-root.js", () => ({
  resolveOpenMehdiPackageRoot: vi.fn().mockResolvedValue("/tmp/openmehdi"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openmehdi/pnpm-lock.yaml",
      markerPath: "/tmp/openmehdi/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("OpenMehdi status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("openmehdi status --all") ||
          l.includes("openmehdi --profile isolated status --all") ||
          l.includes("openmehdi status --all") ||
          l.includes("openmehdi --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OPENMEHDI_GATEWAY_TOKEN = "abcd1234";
    try {

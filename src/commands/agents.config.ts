...
function resolveAgentName(cfg: OpenMehdiConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenMehdiConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenMehdiConfig): AgentSummary[] {

...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENMEHDI_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENMEHDI_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENMEHDI_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENMEHDI_GATEWAY_PASSWORD");
  });

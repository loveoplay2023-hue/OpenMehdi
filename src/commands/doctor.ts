...
function resolveMode(cfg: OpenMehdiConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenMehdi doctor");

  const root = await resolveOpenMehdiPackageRoot({
  ...
  let cfg: OpenMehdiConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("openmehdi configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openmehdi config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openmehdi setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openmehdi doctor --fix")}" to apply changes.`);
  }

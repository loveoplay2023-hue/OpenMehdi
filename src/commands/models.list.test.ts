...
const ensureOpenMehdiModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/openmehdi-agent/auth.json" });
const resolveOpenMehdiAgentDir = vi.fn().mockReturnValue("/tmp/openmehdi-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/openmehdi-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureOpenMehdiModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/openmehdi-agent");
  });

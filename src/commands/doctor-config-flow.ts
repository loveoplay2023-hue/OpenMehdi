...
function stripUnknownConfigKeys(config: OpenMehdiConfig): {
  config: OpenMehdiConfig;
  removed: string[];
} {
  const parsed = OpenMehdiSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: OpenMehdiConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: OpenMehdiConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: OpenMehdiConfig): Promise<{
  config: OpenMehdiConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: OpenMehdiConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: OpenMehdiConfig): {
  config: OpenMehdiConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: OpenMehdiConfig): {
  config: OpenMehdiConfig;
  changes: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".openmehdi");
  const targetPath = path.join(targetDir, "openmehdi.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: OpenMehdiConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("openmehdi doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("openmehdi doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("openmehdi doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("openmehdi doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("openmehdi doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "openmehdi doctor --fix" to remove these keys.');
    }
  }

...
export function requireOpenMehdiAgentDir(): string {
  const agentDir = process.env.OPENMEHDI_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENMEHDI_AGENT_DIR not set");
  }
  return agentDir;
}

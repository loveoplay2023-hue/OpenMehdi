...
const SEARCH_TOOL = "https://docs.openmehdi.ai/mcp.SearchOpenMehdi";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openmehdi.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openmehdi docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openmehdi.ai/");
      runtime.log(`Search: ${formatCliCommand('openmehdi docs "your query"')}`);
    }
    return;
  }

...
import type { OpenMehdiConfig, ConfigValidationIssue } from "./types.js";
import { OpenMehdiSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: OpenMehdiConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OpenMehdiConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenMehdiSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenMehdiConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenMehdiConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as OpenMehdiConfig,
  };
}

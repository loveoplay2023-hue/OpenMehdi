...
import type { ConfigFileSnapshot } from "./types.openmehdi.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.openmehdi/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/openmehdi/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/openmehdi/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/openmehdi/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/openmehdi/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OpenMehdiSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OpenMehdiConfig";
    const hints = mapSensitivePaths(OpenMehdiSchema, "", {});

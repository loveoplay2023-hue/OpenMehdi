...
import type { OpenMehdiConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareOpenMehdiVersions } from "./version.js";
...
function coerceConfig(value: unknown): OpenMehdiConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpenMehdiConfig;
}
...
function stampConfigVersion(cfg: OpenMehdiConfig): OpenMehdiConfig {
...
function warnIfConfigFromFuture(cfg: OpenMehdiConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenMehdiVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenMehdi (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OpenMehdiConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenMehdiConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenMehdiConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenMehdiConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OpenMehdiConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OPENMEHDI_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.OPENMEHDI_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.OPENMEHDI_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.OPENMEHDI_WATCH_SESSION === "string" &&
        deps.env.OPENMEHDI_WATCH_SESSION.trim().length > 0
          ? deps.env.OPENMEHDI_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPENMEHDI_WATCH_COMMAND === "string" &&
        deps.env.OPENMEHDI_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPENMEHDI_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `OPENMEHDI_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENMEHDI_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENMEHDI_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): OpenMehdiConfig {

...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OPENMEHDI_HOME for tilde expansion", () => {
    vi.stubEnv("OPENMEHDI_HOME", "/srv/openmehdi-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/openmehdi-home", "cron", "jobs.json"));
  });
});

...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "openmehdi-cron-" });
}
...
export async function writeSessionStore(
  ...
  const dir = path.join(home, ".openmehdi", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OpenMehdiConfig> = {},
): OpenMehdiConfig {
  const base: OpenMehdiConfig = {
    agents: {
      defaults: {

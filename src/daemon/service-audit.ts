...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.OPENMEHDI_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service OPENMEHDI_GATEWAY_TOKEN does not match gateway.auth.token in openmehdi.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `openmehdi gateway install --force` to sync the token.",
      level: "recommended",
    };
  }

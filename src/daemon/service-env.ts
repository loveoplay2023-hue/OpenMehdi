...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.OPENMEHDI_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel ||
    (process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.OPENMEHDI_STATE_DIR;
  const configPath = env.OPENMEHDI_CONFIG_PATH;
  return {
    HOME: env.HOME,
    PATH: buildMinimalServicePath({ env }),
    OPENMEHDI_PROFILE: profile,
    OPENMEHDI_STATE_DIR: stateDir,
    OPENMEHDI_CONFIG_PATH: configPath,
    OPENMEHDI_GATEWAY_PORT: String(port),
    OPENMEHDI_GATEWAY_TOKEN: token,
    OPENMEHDI_LAUNCHD_LABEL: resolvedLaunchdLabel,
    OPENMEHDI_SYSTEMD_UNIT: systemdUnit,
    OPENMEHDI_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    OPENMEHDI_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  return {
    ...
    OPENMEHDI_STATE_DIR: stateDir,
    OPENMEHDI_CONFIG_PATH: configPath,
    OPENMEHDI_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    OPENMEHDI_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    OPENMEHDI_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    OPENMEHDI_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    OPENMEHDI_LOG_PREFIX: "node",
    OPENMEHDI_SERVICE_MARKER: NODE_SERVICE_MARKER,
    OPENMEHDI_SERVICE_KIND: NODE_SERVICE_KIND,
    OPENMEHDI_SERVICE_VERSION: VERSION,
  };
}

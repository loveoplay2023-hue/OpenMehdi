...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.openmehdi.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", OPENMEHDI_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    ...
    const label = "ai.openmehdi.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when OPENMEHDI_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.openmehdi.gateway.plist",
    );
  });

  it("uses profile-specific label when OPENMEHDI_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", OPENMEHDI_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.openmehdi.jbphoenix.plist",
    );
  });

  it("prefers OPENMEHDI_LAUNCHD_LABEL over OPENMEHDI_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      OPENMEHDI_PROFILE: "jbphoenix",
      OPENMEHDI_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from OPENMEHDI_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      OPENMEHDI_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty OPENMEHDI_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      OPENMEHDI_PROFILE: "myprofile",
      OPENMEHDI_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.openmehdi.myprofile.plist",
    );
  });
});

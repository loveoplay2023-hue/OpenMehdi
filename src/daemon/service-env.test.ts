...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENMEHDI_GATEWAY_PORT).toBe("18789");
    expect(env.OPENMEHDI_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENMEHDI_SERVICE_MARKER).toBe("openmehdi");
    expect(env.OPENMEHDI_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENMEHDI_SERVICE_VERSION).toBe("string");
    expect(env.OPENMEHDI_SYSTEMD_UNIT).toBe("openmehdi-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENMEHDI_LAUNCHD_LABEL).toBe("ai.openmehdi.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENMEHDI_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENMEHDI_SYSTEMD_UNIT).toBe("openmehdi-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENMEHDI_LAUNCHD_LABEL).toBe("ai.openmehdi.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openmehdi"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPENMEHDI_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openmehdi-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", OPENMEHDI_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openmehdi"));
  });

  it("uses OPENMEHDI_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OPENMEHDI_STATE_DIR: "/var/lib/openmehdi" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openmehdi"));
  });

  it("expands ~ in OPENMEHDI_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OPENMEHDI_STATE_DIR: "~/openmehdi-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openmehdi-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OPENMEHDI_STATE_DIR: "C:\\State\\openmehdi" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\openmehdi");
  });
});

...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.openmehdiCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.openmehdiCanvasA2UIAction &&
        typeof window.openmehdiCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.openmehdiSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("openmehdi:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android OpenMehdi node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.openmehdiSendUserAction === "function"
        ? window.openmehdiSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPENMEHDI_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.OPENMEHDI_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>OpenMehdi Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;

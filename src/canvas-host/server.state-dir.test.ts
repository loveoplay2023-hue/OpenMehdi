...
describe("canvas host state dir defaults", () => {
  ...
  it("uses OPENMEHDI_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openmehdi-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("OpenMehdi Canvas");
    } finally {
      await handler.close();

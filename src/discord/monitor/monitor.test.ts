...
describe("agent components", () => {
  const createCfg = (): OpenMehdiConfig => ({}) as OpenMehdiConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): OpenMehdiConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/openmehdi-sessions-test.json");
  });

...
async function main() {
  ...
  const { ensureOpenMehdiCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[openmehdi] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });

...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { OPENMEHDI_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { OPENMEHDI_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports OPENMEHDI_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { OPENMEHDI_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { OPENMEHDI_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { OPENMEHDI_SMOKE: "nope" })).toBe("qr");
  });
});

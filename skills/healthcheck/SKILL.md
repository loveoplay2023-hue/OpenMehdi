---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenMehdi deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenMehdi cron scheduling for periodic checks, or version status checks on a machine running OpenMehdi (laptop, workstation, Pi, VPS).
---

# OpenMehdi Host Hardening

## Overview

Assess and harden the host running OpenMehdi, then align it to a user-defined risk tolerance without breaking access. Use OpenMehdi security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenMehdi changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenMehdi gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenMehdi can access sensitive data.
...
### 2) Run OpenMehdi security audits (read-only)

As part of the default read-only checks, run `openmehdi security audit --deep`. Only offer alternatives if the user requests them:

1. `openmehdi security audit` (faster, non-probing)
2. `openmehdi security audit --json` (structured output)

Offer to apply OpenMehdi safe defaults (numbered):

1. `openmehdi security audit --fix`

Be explicit that `--fix` only tightens OpenMehdi defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenMehdi version/update status (read-only)

As part of the default read-only checks, run `openmehdi update status`.
...
- Credential hygiene notes (location of OpenMehdi creds, prefer disk encryption)
...
- OpenMehdi security audit (re-run)
...
After OpenMehdi install or first hardening pass, run at least one baseline audit and version check:

- `openmehdi security audit`
- `openmehdi security audit --deep`
- `openmehdi update status`

Ongoing monitoring is recommended. Use the OpenMehdi cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `openmehdi cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `openmehdi update status`
...
Before creating, `openmehdi cron list` and match on exact `name`. If found, `openmehdi cron edit <id> ...`.
If not found, `openmehdi cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `openmehdi update status` (preferred for source checkouts and channels)
2. `npm view openmehdi version` (published npm version)

## OpenMehdi command accuracy

Use only supported commands and flags:

- `openmehdi security audit [--deep] [--fix] [--json]`
- `openmehdi status` / `openmehdi status --deep`
- `openmehdi health --json`
- `openmehdi update status`
- `openmehdi cron add|list|runs|run`

Do not invent CLI flags or imply OpenMehdi enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenMehdi compaction:

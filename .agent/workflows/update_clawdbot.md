---
description: Update openmehdi from upstream when branch has diverged (ahead/behind)
---

# openmehdi Upstream Sync Workflow
...
```bash
# Kill running app
pkill -x "openmehdi" || true

# Move old version
mv /Applications/openmehdi.app /tmp/openmehdi-backup.app

# Install new build
cp -R dist/openmehdi.app /Applications/

# Launch
open /Applications/openmehdi.app
```
...
pnpm openmehdi health

# Verify no zombie processes
ps aux | grep -E "(openmehdi|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm openmehdi agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update openmehdi.json with fallback chains
...
pnpm openmehdi agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"

...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openmehdi-e2e-${name}-`));
  const configDir = path.join(homeDir, ".openmehdi");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENMEHDI_SKIP_BROWSER_CONTROL_SERVER: "1",
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(
          ...
          { "x-openmehdi-token": gwA.hookToken },
        ),
        postJson(
          ...
          { "x-openmehdi-token": gwB.hookToken },
        ),
      ]);

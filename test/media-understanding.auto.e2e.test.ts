...
import type { OpenMehdiConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("openmehdi-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("openmehdi-bin-sherpa-");
      const modelDir = await makeTempDir("openmehdi-sherpa-model-");
      ...
      const cfg: OpenMehdiConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openmehdi-bin-whispercpp-");
      const modelDir = await makeTempDir("openmehdi-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openmehdi-bin-gemini-");
      ...
      const cfg: OpenMehdiConfig = { tools: { media: { image: {} } } };

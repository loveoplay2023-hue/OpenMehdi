...
type EnvSnapshot = {
  ...
  openmehdiHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    openmehdiHome: process.env.OPENMEHDI_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OPENMEHDI_HOME", snapshot.openmehdiHome);
  restoreKey("OPENMEHDI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OPENMEHDI_HOME.
  delete process.env.OPENMEHDI_HOME;
  process.env.OPENMEHDI_STATE_DIR = path.join(base, ".openmehdi");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "openmehdi-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".openmehdi", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
